...
background-size: cover;
	background-position: center;'
              ...
              </li>`).join(""),H=document.querySelector(".modal-container");document.addEventListener("DOMContentLoaded",function(){document.body.addEventListener("click",e=>{const t=e.target.closest(".start-button");if(t){const s=t.attributes["data-id"].value;ae(s)}})});function ae(e){const t=W(e);H.innerHTML=re(t);const s=document.querySelector(".modal-backdrop"),a=document.querySelector(".modal-close"),n=document.querySelector(".modal-favorite"),o=ie({modal:s,modalCloseButton:a});a.addEventListener("click",o),n.addEventListener("click",()=>{J(e)}),s.classList.add("is-open"),a.style.display="block",document.addEventListener("keydown",A({closeFn:o})),s.addEventListener("click",D({modal:s,closeFn:o}))}function A({closeFn:e}){return function(t){t.key==="Escape"&&e()}}function D({modal:e,closeFn:t}){return function(s){s.target===e&&t()}}function ie({modal:e,modalCloseButton:t}){return function(){e.classList.remove("is-open"),t.style.display="none",document.removeEventListener("keydown",A),e.removeEventListener("click",D),H.innerHTML=""}}function re({id:e,gifUrl:t,name:s,rating:a,target:n,bodyPart:o,equipment:d,popularity:r,burnedCalories:c,time:f,description:N}){const j=a-parseInt(a)>.5?Math.round(a):Math.floor(a);return`<div class="modal-backdrop">
      <div class="modal">
