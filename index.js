...
background-size: cover;
	background-position: center;'
              ...
              </li>`).join(""),G=document.querySelector(".modal-container");document.addEventListener("DOMContentLoaded",function(){document.body.addEventListener("click",e=>{const t=e.target.closest(".start-button");if(t){const s=t.attributes["data-id"].value;de(s)}})});function de(e){const t=ee(e);G.innerHTML=ue(t);const s=document.querySelector(".modal-backdrop"),a=document.querySelector(".modal-close"),c=document.querySelector(".modal-favorite"),n=le({modal:s,modalCloseButton:a});a.addEventListener("click",n),c.addEventListener("click",()=>{te(e)}),s.classList.add("is-open"),a.style.display="block",document.addEventListener("keydown",O({closeFn:n})),s.addEventListener("click",U({modal:s,closeFn:n}))}function O({closeFn:e}){return function(t){t.key==="Escape"&&e()}}function U({modal:e,closeFn:t}){return function(s){s.target===e&&t()}}function le({modal:e,modalCloseButton:t}){return function(){e.classList.remove("is-open"),t.style.display="none",document.removeEventListener("keydown",O),e.removeEventListener("click",U),G.innerHTML=""}}function ue({id:e,gifUrl:t,name:s,rating:a,target:c,bodyPart:n,equipment:l,popularity:r,burnedCalories:d,time:k,description:K}){const V=a-parseInt(a)>.5?Math.round(a):Math.floor(a);return`<div class="modal-backdrop">
      <div class="modal">
    ...
    </div>`}document.addEventListener("DOMContentLoaded",()=>{ie(),re()});
