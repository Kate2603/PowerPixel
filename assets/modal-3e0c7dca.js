  ...
  <div class="filtered-categories-content">
...
</li>`,t.querySelector(".remove-button").addEventListener("click",r=>{const s=r.currentTarget.dataset.id;b(s),h()}),t}console.log({1:l(),2:l()==="favorites"});l()==="favorites"&&(console.log(3),document.addEventListener("DOMContentLoaded",h));const D=document.querySelector(".modal-container");document.addEventListener("DOMContentLoaded",function(){document.body.addEventListener("click",e=>{const t=e.target.closest(".start-button");if(t){const o=t.attributes["data-id"].value;N(o)}})});async function N(e){const{_id:t,...o}=await O(e);D.innerHTML=Q({id:e,...o});const r=document.querySelector(".modal-backdrop"),s=document.querySelector(".modal-close"),n=document.querySelector(".modal-favorite"),a=J({modal:r,modalCloseButton:s}),{result:g}=w(e);L({result:g,modalFavorite:n}),s.addEventListener("click",a),n.addEventListener("click",()=>{const v=b(e);L({result:v,modalFavorite:n}),l()==="favorites"&&(h(),a())}),r.classList.add("is-open"),s.style.display="block",document.addEventListener("keydown",M({closeFn:a})),r.addEventListener("click",q({modal:r,closeFn:a}))}const V={[c.ADDED]:_,[c.REMOVED]:j};function L({result:e,modalFavorite:t}){const o=V[e];o({modalFavorite:t})}function _({modalFavorite:e}){e.innerHTML=`<span>Remove from favorites</span>
    <svg class="remove-icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" style="fill: currentColor;fill: currentColor;">
      <use href="./img/sprite.svg#icon-trash"></use>
                ...
                </svg>`}function M({closeFn:e}){return function(t){t.key==="Escape"&&e()}}function q({modal:e,closeFn:t}){return function(o){o.target===e&&t()}}function J({modal:e,modalCloseButton:t}){return function(){e.classList.remove("is-open"),t.style.display="none",document.removeEventListener("keydown",M),e.removeEventListener("click",q),D.innerHTML=""}}function Q({id:e,gifUrl:t,name:o,rating:r,target:s,bodyPart:n,equipment:a,popularity:g,burnedCalories:v,time:T,description:k}){const C=r-parseInt(r)>.5?Math.round(r):Math.floor(r);return`<div class="modal-backdrop">
      <div class="modal">
        ...
        <div class="modal-inner">
          ...
          <div class="modal-content">
            ...
            <div class="modal-details">
              ...
              <p><strong>Burned Calories:</strong> ${v}/${T} min</p>
            </div>
...
//# sourceMappingURL=modal-3e0c7dca.js.map
