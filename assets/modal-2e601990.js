  ...
  <div class="filtered-categories-content">
...
</li>`,t.querySelector(".remove-button").addEventListener("click",r=>{const s=r.currentTarget.dataset.id;w(s),p()}),t}function N(){p()}m()==="favorites"&&document.addEventListener("DOMContentLoaded",N);const S=document.querySelector(".modal-container");document.addEventListener("DOMContentLoaded",function(){document.body.addEventListener("click",e=>{const t=e.target.closest(".start-button");if(t){const o=t.attributes["data-id"].value;V(o)}})});async function V(e){const{_id:t,...o}=await C(e);S.innerHTML=G({id:e,...o});const r=document.querySelector(".modal-backdrop"),s=document.querySelector(".modal-close"),n=document.querySelector(".modal-favorite"),a=Q({modal:r,modalCloseButton:s}),{result:u}=E(e);L({result:u,modalFavorite:n}),s.addEventListener("click",a),n.addEventListener("click",()=>{const g=w(e);L({result:g,modalFavorite:n}),m()==="favorites"&&(p(),a())}),r.classList.add("is-open"),s.style.display="block",document.addEventListener("keydown",M({closeFn:a})),r.addEventListener("click",D({modal:r,closeFn:a}))}const _={[c.ADDED]:J,[c.REMOVED]:j};function L({result:e,modalFavorite:t}){const o=_[e];o({modalFavorite:t})}function J({modalFavorite:e}){e.innerHTML=`<span>Remove from favorites</span>
    <svg class="remove-icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" style="fill: currentColor;fill: currentColor;">
      <use href="./img/sprite.svg#icon-trash"></use>
                ...
                </svg>`}function M({closeFn:e}){return function(t){t.key==="Escape"&&e()}}function D({modal:e,closeFn:t}){return function(o){o.target===e&&t()}}function Q({modal:e,modalCloseButton:t}){return function(){e.classList.remove("is-open"),t.style.display="none",document.removeEventListener("keydown",M),e.removeEventListener("click",D),S.innerHTML=""}}function G({id:e,gifUrl:t,name:o,rating:r,target:s,bodyPart:n,equipment:a,popularity:u,burnedCalories:g,time:q,description:T}){const k=r-parseInt(r)>.5?Math.round(r):Math.floor(r);return`<div class="modal-backdrop">
      <div class="modal">
    ...
    </div>`}export{te as a,Z as b,z as c,Y as d,X as e,W as f,ee as g,oe as h,U as s};
//# sourceMappingURL=modal-2e601990.js.map
